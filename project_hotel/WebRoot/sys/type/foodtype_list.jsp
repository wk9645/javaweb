			...
			<tbody id="TableData">
					<c:choose>
						<c:when test="${not empty requestScope.listFoodType}">
							...
							<c:forEach var="dishType" items="${requestScope.listFoodType}">
								<tr>
									<td>${dishType.id }</td>
									<td>${dishType.typeName }</td>
									<td>
										<a href="${pageContext.request.contextPath }/DishTypeServlet?id=${dishType.id}&method=viewUpdate" class="FunctionButton">更新</a> 
										<a href="${pageContext.request.contextPath }/DishTypeServlet?id=${dishType.id}&method=delete" class="FunctionButton">删除</a>
									</td>
								</tr>
							</c:forEach>
